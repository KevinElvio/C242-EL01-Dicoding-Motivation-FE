...
export default function ReminderCard({ reminder, index, className }) {
  const navigate = useNavigate();
  return (
    <div
      onClick={() =>
        navigate(`/new-features/reminder-schedule/detail/${index}`)
      }
      ...
      <div className="flex text-lg gap-4 text-theme-base flex-1 items-center justify-stretch w-full">
        <div className="font-semibold rounded-lg border border-theme-base px-4 py-2 flex-1">
          {reminder.frequency == "once"
            ? moment(reminder.time.time).format("yyyy-MM-DD hh:mm")
            : reminder.time.time}
        </div>
        <div className="font-semibold rounded-lg border border-theme-base px-4 py-2 flex-1">
          {reminder.frequency}
